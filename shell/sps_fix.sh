...
if [ -d $tomcatPath ]; then
	# get pid of webManager tomcat
	tomcatID=$(ps -ef|grep $tomcatPath |grep -v 'grep'|awk '{print $2}')
	
	if [ $tomcatID ]; then
		echo "tomcat is running, stop it"
		/etc/init.d/tomcat stop
		sleep 5s
		
		# kill tomcat if it still alive
		tomcatID=$(ps -ef|grep $tomcatPath |grep -v 'grep'|awk '{print $2}')
	...
	# delete old war files before deploy new ones
	if [ -e $applicationDeployFolder/$applicationFile ]; then
		echo 'delete old webManager.war file'
		rm $applicationDeployFolder/$applicationFile 
	fi
	if [ -e $applicationDeployFolder/$webserviceFile ]; then
		echo 'delete old sps.war file'
		rm $applicationDeployFolder/$webserviceFile
	fi
	...
	if [ -e $applicationDeployFolder/$applicationFolder ]; then
		echo 'delete webManager folder'
		rm -r $applicationDeployFolder/$applicationFolder
	fi
	if [ -e $applicationDeployFolder/$webserviceFolder ]; then
		echo 'delete sps folder'
		rm -r $applicationDeployFolder/$webserviceFolder
	fi
	...
	# configure software rsyslog-pgsql
	echo "configure rsyslog-pgsql..."
	cp $sourceFolder/rsyslog/pgsql.conf /etc/rsyslog.d/
	cp $sourceFolder/rsyslog/rsyslog.conf /etc/rsyslog.conf
	
	# init DB
	tar -zvxf $sourceFolder/deploy-base.tar.gz
	tar -zvxf $sourceFolder/deploy-dlp.tar.gz
	tar -zvxf $sourceFolder/deploy-swg.tar.gz
	cd $filepath/deploy-base
	bash ./deploy.sh
	cd $filepath/deploy-dlp
	bash ./deploy.sh
	cd $filepath/deploy-swg
	bash ./deploy.sh
...
LJS
	...
	# configure shmmax and shmall for postgresql
    suggest_max=100663296
	echo "shmmax should be $suggest_max "
	sed -i '/kernel.shmmax=/d' /etc/sysctl.conf 
	echo  "kernel.shmmax=$suggest_max">>/etc/sysctl.conf
	sed -i '/max_connections =/d' /etc/postgresql/9.1/main/postgresql.conf 
	echo "append new config to postgresql.conf/"
	echo  "max_connections = 1024">> /etc/postgresql/9.1/main/postgresql.conf
	#copy skyguardsetup.exe to sps folder
